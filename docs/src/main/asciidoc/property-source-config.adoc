...
set `spring.cloud.kubernetes.config.retry.enabled=true`. Then you need to add `spring-retry`
...
As with the `ConfigMap` property sources, first you need to set `spring.cloud.kubernetes.secrets.retry.enabled=true`.
