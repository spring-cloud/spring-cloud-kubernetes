...
functionality will work only via `POLLING` reload. If that is not an option, you will need to make a `POST` request to the `/actuator/refresh` endpoint or restart/redeploy the application.
