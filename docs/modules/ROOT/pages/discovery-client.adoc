...
- Only take services that match certain service labels. This property is specified with: `spring.cloud.kubernetes.discovery.service-labels`. It accepts a `Map` and only those services that have such labels (as seen in `metadata.labels` in the service definition) will be taken into account. Since we use listers to get the services ( basically a cache that is populated async ), these labels are particularly useful when your namespaces have a big number of services and using labels, you make the cache smaller.
