...
By default, Configuration Watcher monitors only ConfigMaps and Secrets that carry these labels:
...
You can change which sources are monitored by configuring label selectors:
...
These properties can also be provided through environment variables in the deployment:
