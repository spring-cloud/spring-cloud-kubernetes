...
Setting `spring.cloud.kubernetes.configuration.watcher.ha.enabled=true` also enables the native
Kubernetes leader election used to choose the active watcher replica. Setting
`spring.cloud.kubernetes.leader.election.enabled=true` without enabling Configuration Watcher HA
does not enable HA for the watcher.

When HA is enabled, the Configuration Watcher starts its ConfigMap and Secret informers only after
it becomes the leader, and stops them when it loses leadership. A replacement leader starts from
the current Kubernetes state; the watcher does not persist informer resource-version checkpoints.

The service account used by the Configuration Watcher needs permission to read and update the
leader-election `Lease`. Add the following rule to the Role:
...
You can also target applications by labels instead of by name.
To do that, add one of these annotations:

- `spring.cloud.kubernetes.configmap.labels`
- `spring.cloud.kubernetes.secret.labels`

Each annotation takes a comma-separated list of `key=value` pairs.
...
When one of these annotations is present, the HTTP implementation and the messaging implementation use the Spring Cloud Kubernetes Discovery Client to fetch candidate ServiceInstance entries and match the provided labels against ServiceInstance::getMetadata`.

This makes labels-based targeting the labels-based alternative to:
...
- `*.apps` targets discovered `ServiceInstance` entries by name
- `*.labels` targets discovered `ServiceInstance` entries by metadata labels

IMPORTANT: `spring.cloud.kubernetes.discovery.metadata.add-labels` must be enabled so that labels are present in
`ServiceInstance::getMetadata`.

If `spring.cloud.kubernetes.discovery.metadata.labels-prefix` is set, the keys used in:

- `spring.cloud.kubernetes.configmap.labels`
- `spring.cloud.kubernetes.secret.labels`

must match the prefixed keys exposed in the metadata.

If both `*.apps` and `*.labels` annotations are present, `*.labels` takes precedence.
...
When `spring.cloud.kubernetes.configmap.labels` or `spring.cloud.kubernetes.secret.labels` is present, the messaging implementation also resolves
candidate `ServiceInstance` entries from the discovery client and matches labels against `ServiceInstance::getMetadata`. For each matching
service id, it publishes a single bus event.
...
NOTE: Any pod connected to the messaging broker and using Spring Cloud Bus has the potential for a
refresh event being sent to it.  This means if a pod in another Kubernetes namespace is connected to
the broker and uses Spring Cloud Bus it could be refreshed.  If admins should be mindful of
the pods that are connected to the broker if they want to prevent them from receiving refresh
events.
