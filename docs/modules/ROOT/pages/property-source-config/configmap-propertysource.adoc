...
thing to notice here is that unlike reading a configmap by name, this can result in _multiple_ config maps read. The same feature is supported for secrets.

Another important point is that if you plan to use `labels` searching coupled with `includeProfileSpecificSources` functionality, we strongly encourage such configmaps (and secrets) names to have zero or one dashes (`-`) (as defined in the `.metadata.name`). The reason for this is simple, suppose you define two configmaps that have the same labels:
...
when we look up for them, how do we know what is the profile name : `app`, `app-dev`, or simply `dev`? Or may be these names don't have a profile at all. The only reasonable assumption we can do is that everything that follows a dash is a profile name, be that active or not. So the above should be changed to:
...
Then `my_app` is the non-profile based name and `my_app-dev` is the source we will be using _only_ if `dev` is an active profile.
