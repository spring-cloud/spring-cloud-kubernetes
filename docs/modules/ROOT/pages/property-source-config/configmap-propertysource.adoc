...
Another important point is that if you plan to use `labels` searching, we strongly encourage such configmaps (and secrets) names to have zero or one dashes (`-`) (as defined in the `.metadata.name`). The reason for this is simple, suppose you define two configmaps that have the same labels:
