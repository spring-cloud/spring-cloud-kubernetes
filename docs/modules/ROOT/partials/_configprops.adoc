...
|spring.cloud.kubernetes.config.read-type | `+++BATCH+++` | 
...
|spring.cloud.kubernetes.discovery.filter |  | SpEL expression used to filter Services after they’re fetched from the Kubernetes API server.
Example (matches namespaces ending with A):
`#root.metadata.namespace matches '^.+A$'`
This requires knowing the exact object structure of the Service type being evaluated (Fabric8 Service or Kubernetes Java client V1Service), because field/property names differ between implementations.
...
|spring.cloud.kubernetes.leader.election.lease-duration | `+++15s+++` | 
|spring.cloud.kubernetes.leader.election.lock-name | `+++spring-k8s-leader-election-lock+++` | 
|spring.cloud.kubernetes.leader.election.lock-namespace | `+++default+++` | 
|spring.cloud.kubernetes.leader.election.publish-events | `+++true+++` | 
|spring.cloud.kubernetes.leader.election.renew-deadline | `+++10s+++` | 
|spring.cloud.kubernetes.leader.election.restart-on-failure | `+++true+++` | 
|spring.cloud.kubernetes.leader.election.retry-period | `+++2s+++` | 
|spring.cloud.kubernetes.leader.election.use-config-map-as-lock | `+++false+++` | 
...
|spring.cloud.kubernetes.secrets.read-type | `+++BATCH+++` | 
