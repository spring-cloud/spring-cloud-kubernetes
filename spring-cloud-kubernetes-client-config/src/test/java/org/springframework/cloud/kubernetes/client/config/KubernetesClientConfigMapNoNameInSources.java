...
import org.springframework.cloud.kubernetes.client.config.name_not_provided_in_sources.NameNotProvidedInSourcesApp;
import org.springframework.test.web.reactive.server.WebTestClient;

/**
 * The stub data for this test is in : ConfigMapNameInSourcesNotPresent
 *
 * @author wind57
 */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, classes = NameNotProvidedInSourcesApp.class,
		properties = { "spring.cloud.bootstrap.name=config-map-name-not-provided-in-sources",
				"config.map.name.in.sources.not.present.stub=true", "spring.main.cloud-platform=KUBERNETES" })
class KubernetesClientConfigMapNoNameInSources {
