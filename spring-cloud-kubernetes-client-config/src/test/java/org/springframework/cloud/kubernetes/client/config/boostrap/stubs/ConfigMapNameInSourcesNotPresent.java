...
@Order(0)
@Configuration
@ConditionalOnProperty("config.map.name.in.sources.not.present.stub")
public class ConfigMapNameInSourcesNotPresent {
	...
	private void stubData() {
		V1ConfigMap one = new V1ConfigMapBuilder()
				.withMetadata(new V1ObjectMetaBuilder().withName("application").withNamespace("spring-k8s")
						.withResourceVersion("1").build())
				.addToData(Collections.singletonMap("configmap.property", "one")).build();

		V1ConfigMapList allConfigMaps = new V1ConfigMapList();
		allConfigMaps.setItems(Collections.singletonList(one));
