...
public final class Util {
	...
	public void deleteAndWait(String namespace, V1Deployment deployment, V1Service service,
		...
		if (deployment != null) {
			try {
				...
				coreV1Api.deleteCollectionNamespacedPod(namespace).execute();
		...
		try {
			...
			if (ingress != null) {
				...
				networkingV1Api.deleteNamespacedIngress(ingressName, namespace).execute();
	...
	public void setUpClusterWideClusterRoleBinding(String serviceAccountNamespace) {
		...
		try {
			...
			notExistsHandler(() -> rbacApi.readClusterRoleBinding(clusterRoleBinding.getMetadata().getName()).execute(),
	...
	public static void patchWithMerge(String deploymentName, String namespace, String patchBody,
		...
		try {
			PatchUtils.patch(V1Deployment.class,
					() -> new AppsV1Api().patchNamespacedDeployment(deploymentName, namespace, new V1Patch(patchBody))
						.buildCall(null),
	...
	public static void patchWithReplace(String imageName, String deploymentName, String namespace, String patchBody,
		...
		try {
			PatchUtils.patch(V1Deployment.class,
					() -> new AppsV1Api().patchNamespacedDeployment(deploymentName, namespace, new V1Patch(body))
						.buildCall(null),
	...
	private void waitForIngress(String namespace, V1Ingress ingress) {
		...
		await().timeout(Duration.ofSeconds(90)).pollInterval(Duration.ofSeconds(3)).until(() -> {
			try {
				V1IngressLoadBalancerStatus status = networkingV1Api.readNamespacedIngress(ingressName, namespace)
					.execute()
					.getStatus()
					.getLoadBalancer();
	...
	private void waitForIngressToBeDeleted(String ingressName, String namespace) {
		await().timeout(Duration.ofSeconds(90)).until(() -> {
			try {
				networkingV1Api.readNamespacedIngress(ingressName, namespace).execute();
				return false;
			}
			catch (ApiException e) {
				if (e.getCode() == HttpURLConnection.HTTP_NOT_FOUND) {
