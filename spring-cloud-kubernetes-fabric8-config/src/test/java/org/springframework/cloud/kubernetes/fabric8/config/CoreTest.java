...
abstract class CoreTest {
	...
	public static void setUpBeforeClass(KubernetesClient mockClient) {
		...
		mockClient.configMaps().inNamespace("testns")
			.resource(new ConfigMapBuilder().withNewMetadata().withName("testapp").endMetadata().addToData(data1).build()).create();

		Map<String, String> data2 = new HashMap<>();
		data2.put("amq.user", "YWRtaW4K");
		data2.put("amq.pwd", "MWYyZDFlMmU2N2Rm");
		mockClient.secrets().inNamespace("testns")
			.resource(new SecretBuilder().withNewMetadata().withName("testapp").endMetadata().addToData(data2).build()).create();
