...
import org.springframework.cloud.kubernetes.fabric8.config.example4.SecretPrefixApp;
...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, classes = SecretPrefixApp.class,
		properties = { "spring.cloud.bootstrap.name=secret-prefix" })
...
public class SecretPrefixTests {
	...
	public static void setUpBeforeClass() {
		...
		Secret secret1 = new SecretBuilder().withNewMetadata().withName("name1").withLabels(metadata1).endMetadata()
				.addToData("secret", Base64.getEncoder().encodeToString(SECRET_VALUE_1.getBytes())).build();
		...
		Secret secret2 = new SecretBuilder().withNewMetadata().withName("name2").withLabels(metadata2).endMetadata()
				.addToData("secret", Base64.getEncoder().encodeToString(SECRET_VALUE_2.getBytes())).build();
	...
	public void testSecret1() {
		assertResponse("/prefix-test/name1/secret", SECRET_VALUE_1);
	}

	@Test
	public void testSecret2() {
		assertResponse("/prefix-test/name2/secret", SECRET_VALUE_2);
	}
