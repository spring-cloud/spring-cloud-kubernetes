...
class Fabric8ConfigUtilsNamespacedBatchReadTests {
	...
	@AfterEach
	void afterEach() {
		Fabric8SourcesBatchRead.discardSecrets();
	...
	void testSecretDataByLabelsTwoSecretsFound() {
		...
		LinkedHashMap<String, Map<String, Object>> data = result.data();
		Assertions.assertThat(data).hasSize(2);
		Map<String, Object> mySecretData = result.data().get("my-secret");
		Assertions.assertThat(mySecretData.get("property")).isEqualTo("value");

		Map<String, Object> mySecretData2 = result.data().get("my-secret-2");
		Assertions.assertThat(mySecretData2.get("property-2")).isEqualTo("value-2");
	}
	...
	void testSecretDataByLabelsThreeSecretsFound() {
		...
		Map<String, Object> dataOne = result.data().get("blue-circle-secret");
		Assertions.assertThat(dataOne).containsExactlyInAnyOrderEntriesOf(Map.of("one", "1"));

		Map<String, Object> dataTwo = result.data().get("blue-square-secret");
		Assertions.assertThat(dataTwo).containsExactlyInAnyOrderEntriesOf(Map.of("two", "2"));
	 ...
	 */
	@Test
	void testConfigMapsDataByNameFoundNo() {
		client.configMaps()
			.inNamespace("spring-k8s")
	 ...
	 */
	@Test
	void testConfigMapsByNameNotFound() {
		client.configMaps()
			.inNamespace("spring-k8s")
			.resource(new ConfigMapBuilder().withMetadata(new ObjectMetaBuilder().withName("my-config-map").build())
				.build())
	...
	void testConfigMapDataByNameTwoFound() {
		...
		Map<String, Object> data = result.data().get("my-config-map");
		Assertions.assertThat(data).contains(Map.entry("property", "value"));

		Map<String, Object> data2 = result.data().get("my-config-map-2");
		Assertions.assertThat(data2).contains(Map.entry("property-2", "value-2"));
	}
