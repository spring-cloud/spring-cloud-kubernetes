...
class Fabric8ServiceListSupplierTests {
	...
	@SuppressWarnings("unchecked")
	private final FilterWatchListDeletable<Service, ServiceList, ServiceResource<Service>> filterWatchListDeletable = Mockito
	...
	void testPositiveMatch() {
		...
		when(this.namespaceOperation.withField("metadata.name", "test-service")).thenReturn(filterWatchListDeletable);
		when(filterWatchListDeletable.list())
