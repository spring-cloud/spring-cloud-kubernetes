...
public final class Fabric8ReactiveDiscoveryClientAutoConfiguration {
	...
	Fabric8ReactiveDiscoveryClient fabric8ReactiveDiscoveryClient(KubernetesClient client,
		...
		Fabric8DiscoveryClient fabric8DiscoveryClient = new Fabric8DiscoveryClient(client, serviceListers,
				endpointsListers, serviceInformers, endpointsInformers, properties, predicate);
	...
	Fabric8CacheableReactiveDiscoveryClient fabric8CacheableReactiveDiscoveryClient(KubernetesClient client,
			...
			List<SharedIndexInformer<Endpoints>> endpointsInformers, KubernetesDiscoveryProperties properties,
			Predicate<Service> predicate) {
		Fabric8DiscoveryClient blockingClient = new Fabric8DiscoveryClient(client, serviceListers, endpointsListers,
				serviceInformers, endpointsInformers, properties, predicate);
	...
	// do not want to allow wiring of it. Nevertheless, we still need an instance of
	// Fabric8DiscoveryClient in order to create the
	// ReactiveDiscoveryClientHealthIndicator and
	// Fabric8CacheableReactiveDiscoveryClient. As such, we create two of such instances
	// in each bean.
