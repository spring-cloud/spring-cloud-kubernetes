...
class ConfigurationWatcherMultipleAppIT {
	...
	private static final String CONFIG_WATCHER_APP_B_IMAGE = "spring-cloud-kubernetes-configuration-watcher-app-b";

	private static final String CONFIG_WATCHER_DEPLOYMENT_APP_A_NAME = "spring-cloud-kubernetes-configuration-watcher-app-a-deployment";

	private static final String CONFIG_WATCHER_DEPLOYMENT_APP_B_NAME = "spring-cloud-kubernetes-configuration-watcher-app-b-deployment";
	...
	private void deployAppA() throws Exception {
		...
		V1Ingress ingress = getAppAIngress();
	...
	private V1Deployment getAppADeployment() throws Exception {
		String urlString = "app-a/spring-cloud-kubernetes-configuration-watcher-app-a-deployment.yaml";
	...
	private V1Service getAppAService() throws Exception {
		return (V1Service) K8SUtils
				.readYamlFromClasspath("app-a/spring-cloud-kubernetes-configuration-watcher-app-a-service.yaml");
	}

	private V1Ingress getAppAIngress() throws Exception {
	...
	private void deployAppB() throws Exception {
		...
		V1Ingress ingress = getAppBIngress();
		networkingApi.createNamespacedIngress(NAMESPACE, ingress, null, null, null);
		k8SUtils.waitForIngress(ingress.getMetadata().getName(), NAMESPACE);
	}

	private V1Deployment getAppBDeployment() throws Exception {
		String urlString = "app-b/spring-cloud-kubernetes-configuration-watcher-app-b-deployment.yaml";
		V1Deployment deployment = (V1Deployment) K8SUtils.readYamlFromClasspath(urlString);
		String image = K8SUtils.getImageFromDeployment(deployment) + ":" + getPomVersion();
		deployment.getSpec().getTemplate().getSpec().getContainers().get(0).setImage(image);
		return deployment;
	...
	private V1Service getAppBService() throws Exception {
		return (V1Service) K8SUtils
				.readYamlFromClasspath("app-b/spring-cloud-kubernetes-configuration-watcher-app-b-service.yaml");
	}

	private V1Ingress getAppBIngress() throws Exception {
		return (V1Ingress) K8SUtils
	...
	private void cleanUpServices() throws Exception {
		api.deleteNamespacedService(CONFIG_WATCHER_APP_A_IMAGE, NAMESPACE, null, null, null, null, null, null);
		api.deleteNamespacedService(CONFIG_WATCHER_APP_B_IMAGE, NAMESPACE, null, null, null, null, null, null);
