...
public class ActuatorRefreshTests {
	...
	private static final String IMAGE_TAG = "2.0.0.BUILD-SNAPSHOT";
...
//		client.setDebugging(true);
	...
	public void testActuatorRefresh() throws Exception {
		...
		V1ConfigMap configMap = new V1ConfigMapBuilder().editOrNewMetadata()
			.withName(CONFIG_WATCHER_WIREMOCK_APP_NAME).addToLabels("spring.cloud.kubernetes.config", "true")
			.endMetadata()
			.addToData("foo", "bar")
			.build();
		...
		await().atMost(Duration.ofMillis(3400)).until(() ->
			!findAll(postRequestedFor(urlEqualTo("/actuator/refresh"))).isEmpty());
	...
	public void after() throws Exception {
		...
		CoreV1Api api = new CoreV1Api();
		api.deleteNamespacedConfigMap(CONFIG_WATCHER_WIREMOCK_APP_NAME, NAMESPACE, null, null,
			null, null, null, null);
	}

	private void waitForDeployment(String deploymentName) {
		await().atMost(90, TimeUnit.SECONDS).until(() -> isDeployentReady(deploymentName));
	}

	private boolean isDeployentReady(String deploymentName) throws ApiException {
		AppsV1Api appsApi = new AppsV1Api();
		V1DeploymentList deployments = appsApi.listNamespacedDeployment(NAMESPACE,null, null,
			"metadata.name=" + deploymentName, null, null, null,
			null, null);
		...
		System.out.println("Available replicas for " + deploymentName + ": " + availableReplicas);
	...
	private void deployConfigWatcher() throws ApiException {
		...
		V1ConfigMap configMap = new V1ConfigMapBuilder().editOrNewMetadata()
			.withName(SPRING_CLOUD_K8S_CONFIG_WATCHER_APP_NAME)
			.endMetadata().addToData("application.properties", "spring.cloud.kubernetes.configuration.watcher.refreshDelay=0")
			.build();
		api.createNamespacedConfigMap(NAMESPACE, configMap, null, null, null);
		...
		int port = 8888;
		createDeployment(SPRING_CLOUD_K8S_CONFIG_WATCHER_DEPLOYMENT_NAME,
			labels, labels, SPRING_CLOUD_K8S_CONFIG_WATCHER_APP_NAME, KIND_IMAGE_WITH_TAG, "IfNotPresent",
			port, port, "/actuator/health/readiness", port, "/actuator/health/liveness",
	...
	private void deployWiremock() throws ApiException {
		...
		int port = 8080;
		V1Deployment wiremockDeployment = createDeployment(CONFIG_WATCHER_WIREMOCK_DEPLOYMENT_NAME, labels, labels,
			CONFIG_WATCHER_WIREMOCK_APP_NAME, "rodolpheche/wiremock", "IfNotPresent", port, port,
			"/__admin/mappings", port, "/__admin/mappings", "default");

		V1Service wiremockService = createService(CONFIG_WATCHER_WIREMOCK_APP_NAME, labels, labels,
			"ClusterIP", "http", port, port);

		NetworkingV1beta1Api networkingApi = new NetworkingV1beta1Api();
		NetworkingV1beta1Ingress ingress = new NetworkingV1beta1IngressBuilder().editOrNewMetadata()
			.withName("nginx-ingress").addToAnnotations("nginx.ingress.kubernetes.io/rewrite-target", "/$2")
			.endMetadata()
			.editOrNewSpec()
			.addNewRule().editOrNewHttp().addNewPath()
			.withNewPath("/wiremock(/|$)(.*)").editOrNewBackend().withNewServiceName(CONFIG_WATCHER_WIREMOCK_APP_NAME)
			.withNewServicePort(port).endBackend()
			.endPath()
			.endHttp()
			.endRule()
			.endSpec()
	...
	private V1Service createService(String name, Map<String, String> labels, Map<String, String> specSelectors,
		String type, String portName, int port, int targetPort) throws ApiException {
		CoreV1Api api = new CoreV1Api();
		V1Service wiremockService = new V1ServiceBuilder().editOrNewMetadata().withName(name).addToLabels(labels).endMetadata()
			.editOrNewSpec().addToSelector(specSelectors).withNewType(type).addNewPort().withName(portName).withPort(port).withNewTargetPort(targetPort)
			.endPort()
			.endSpec()
			.build();
	...
	private V1Deployment createDeployment(String name, Map<String, String> selectorMatchLabels,
		Map<String, String> templateMetadataLabels, String containerName,
		String image, String pullPolicy, int containerPort, int readinessProbePort, String readinessProbePath,
		int livenessProbePort, String livenessProbePath, String serviceAccountName) throws ApiException {
		AppsV1Api appsApi = new AppsV1Api();


		V1Deployment wiremockDeployment = new V1DeploymentBuilder().editOrNewMetadata().withName(name).endMetadata()
			.editOrNewSpec().withNewSelector().addToMatchLabels(selectorMatchLabels)
			.endSelector()
			.editOrNewTemplate()
			.editOrNewMetadata().addToLabels(templateMetadataLabels).endMetadata()
			.editOrNewSpec()
			.withServiceAccountName(serviceAccountName)
			.addNewContainer().withName(containerName).withImage(image).withImagePullPolicy(pullPolicy).addNewPort().withContainerPort(containerPort).endPort()
			.editOrNewReadinessProbe().editOrNewHttpGet().withNewPort(readinessProbePort).withNewPath(readinessProbePath).endHttpGet().endReadinessProbe()
			.editOrNewLivenessProbe().editOrNewHttpGet().withNewPort(livenessProbePort).withNewPath(livenessProbePath).endHttpGet().endLivenessProbe().endContainer()
			.endSpec()
			.endTemplate()
			.endSpec()
			.build();
