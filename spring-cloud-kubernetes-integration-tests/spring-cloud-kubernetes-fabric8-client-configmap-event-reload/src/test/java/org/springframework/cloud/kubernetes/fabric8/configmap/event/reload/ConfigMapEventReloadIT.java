...
class ConfigMapEventReloadIT {
	...
	private static void deleteManifests() {
		...
		try {
			...
			client.configMaps().inNamespace(NAMESPACE).withName(leftConfigMapName).delete();
			client.configMaps().inNamespace(NAMESPACE).withName(rightConfigMapName).delete();
			if (rightWithLabelConfigMapName != null) {
				client.configMaps().inNamespace(NAMESPACE).withName(rightWithLabelConfigMapName).delete();
			}
